...
func rowToEvent(row *excelRow) *GenconEvent {
	...
	return NormalizeEvent(&GenconEvent{
		...
		Year:                 year,
		Active:               true,
		Group:                strings.TrimSpace(cells[1].String),
		Title:                parseCellToString(cells[2]),
		ShortDescription:     strings.TrimSpace(cells[3].String),
		LongDescription:      strings.TrimSpace(cells[4].String),
		EventType:            strings.TrimSpace(cells[5].String),
		GameSystem:           strings.TrimSpace(cells[6].String),
		RulesEdition:         strings.TrimSpace(cells[7].String),
		MinPlayers:           (int)(cells[8].Number),
		MaxPlayers:           (int)(cells[9].Number),
		AgeRequired:          strings.TrimSpace(cells[10].String),
		ExperienceRequired:   strings.TrimSpace(cells[11].String),
		MaterialsProvided:    strings.TrimSpace(cells[12].String) == "Yes",
		StartTime:            startTime,
		Duration:             duration,
		EndTime:              endTime,
		GMNames:              strings.TrimSpace(cells[17].String),
		Website:              strings.TrimSpace(cells[18].String),
		Email:                strings.TrimSpace(cells[19].String),
		Tournament:           strings.TrimSpace(cells[20].String) == "Yes",
		RoundNumber:          (int)(cells[21].Number),
		TotalRounds:          (int)(cells[22].Number),
		MinPlayTime:          (int)(60 * cells[23].Number),
		AttendeeRegistration: strings.TrimSpace(cells[24].String),
		Cost:                 (int)(cells[25].Number),
		Location:             strings.TrimSpace(cells[26].String),
		RoomName:             parseRoom(cells[27]),
		TableNumber:          parseCellToString(cells[28]),
		SpecialCategory:      strings.TrimSpace(cells[29].String),
